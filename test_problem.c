...
void main()
{
	...
	if (player_choice==true_choice  && (sw_st==1||sw_st==3))
	{
		wc++;
		no_change_w++;
	...
	else if(new_choice[final_choice]==true_choice && new_choice[final_choice]!=player_choice && (sw_st==2||sw_st==3))
	{
		wc++;
		change_c_w++;
		printf("\nWin\n");
	...
	else if(player_choice!=true_choice && (sw_st==1||sw_st==3))
	{
		lc++;
		no_change_l++;
		printf("\nFail\n");
